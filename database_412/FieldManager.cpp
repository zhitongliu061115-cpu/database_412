...
bool FieldManager::addField(const std::string& tname, const std::string& fname,
    ...
    new_f.flags = 0;
    if (notNull) new_f.flags |= FIELD_FLAG_NOT_NULL;
...
bool FieldManager::dropField(const std::string& tname, const std::string& fname) {
    ...
    for (size_t i = 0; i < flds.size(); i++) {
        if (std::string(flds[i].name) == fname) { idx = static_cast<int>(i); break; }
    }
    if (idx == -1) { std::cout << "Err: 字段不存在\n"; return false; }
    flds.erase(flds.begin() + idx);
    for (size_t i = idx; i < flds.size(); i++) flds[i].order--;
    saveFields(tname, flds);
